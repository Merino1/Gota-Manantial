...
app = Flask(__name__)
# FIX 2: Secret key de ENV
app.secret_key = os.environ.get("SECRET_KEY", "gota-v11-mxli-dev-key-!cambia-en-render")
# FIX 3: Postgres para Render, sqlite solo local
db_url = os.environ.get("DATABASE_URL", "")
if db_url and db_url.startswith("postgres://"):
    db_url = db_url.replace("postgres://", "postgresql://", 1)
app.config['SQLALCHEMY_DATABASE_URI'] = db_url or f"sqlite:///{os.path.join(os.path.abspath(os.path.dirname(__file__)), 'gota.db')}"
...
class Pedido(db.Model):
    ...
    chofer_id=db.Column(db.Integer, db.ForeignKey('usuario.id'), nullable=True, index=True)
    fecha=db.Column(db.DateTime, default=datetime.now)
class Ubicacion(db.Model):
    id=db.Column(db.Integer, primary_key=True); chofer_id=db.Column(db.Integer, db.ForeignKey('usuario.id'), index=True)
    lat=db.Column(db.Float); lng=db.Column(db.Float); fecha=db.Column(db.DateTime, default=datetime.now, index=True)
class Corte(db.Model):
    id=db.Column(db.Integer, primary_key=True); chofer_id=db.Column(db.Integer, db.ForeignKey('usuario.id'), index=True)
    fecha=db.Column(db.Date, default=date.today, index=True); salio=db.Column(db.Integer, default=0); regreso=db.Column(db.Integer, default=0)
    __table_args__ = (db.UniqueConstraint('chofer_id','fecha'),)
class Descuento(db.Model):
    id=db.Column(db.Integer, primary_key=True); chofer_id=db.Column(db.Integer, db.ForeignKey('usuario.id'), index=True)
    fecha=db.Column(db.Date, default=date.today, index=True); tipo=db.Column(db.String(20)); cantidad=db.Column(db.Integer, default=1); tamano=db.Column(db.String(200)); total=db.Column(db.Float, default=0)
...
def login():
    if request.method=="POST":
        try:
            ...
            db.session.rollback()
            return render_template_string(LOGIN_HTML, error="Error de BD, intenta de nuevo")
...
def panel():
    ...
    if session['rol']=='chofer':
        ...
        if not corte: 
            corte=Corte(chofer_id=session['user_id'], fecha=hoy)
            db.session.add(corte)
            try: db.session.commit()
            except: db.session.rollback(); corte=Corte.query.filter_by(chofer_id=session['user_id'], fecha=hoy).first()
        descuentos=Descuento.query.filter_by(chofer_id=session['user_id'], fecha=hoy).all()
        total_desc=sum(max(0,d.total) for d in descuentos)
        vendidos=max(0, corte.salio-corte.regreso)
        yo=Usuario.query.get(session['user_id']); precio_usar=yo.precio_agua if yo and yo.precio_agua and yo.precio_agua>0 else 22
        total_venta=max(0, vendidos*precio_usar - total_desc)
...
def nuevo_pedido():
    try:
        ...
        p=Pedido(cliente=request.form['cliente'].strip()[:100], direccion=request.form['direccion'].strip()[:200], cantidad=cantidad, precio=precio, chofer_id=cid)
...
def desc_add():
    try:
        ...
        if total < 0: total = 0
        if total > 500: total = 500 # anti-abuso
        d=Descuento(chofer_id=session['user_id'], fecha=date.today(), tipo=request.form['tipo'], cantidad=max(1,int(request.form.get('cantidad',1))), tamano=request.form.get('tamano','')[:200], total=total)
...
def desc_del(id):
    try:
        ...
        if d and d.chofer_id==session['user_id']:
...
def solicitar_precio():
    try:
        ...
        if quedo >= estaba or quedo <=0 or estaba <=0:
            return redirect("/panel") # FIX 5: no permitir precio mas caro
        dif=(estaba-quedo)*cant
        if dif <0 or dif>1000: dif=0
...
def autorizar(id):
    try:
        ...
        if s and s.estado=="Pendiente" and s.precio_quedo < s.precio_estaba:
            s.estado="Autorizado"; d=Descuento(chofer_id=s.chofer_id, fecha=date.today(), tipo="descuento_precio", cantidad=s.cantidad, tamano=f"{s.tamano} ${s.precio_estaba}->{s.precio_quedo} {s.direccion}", total=max(0,s.diferencia))
            db.session.add(d); db.session.commit()
    except: db.session.rollback()
    return redirect("/panel")
...
def nuevo_usuario():
    try:
        if Usuario.query.filter_by(username=request.form['username'].strip()).first():
            return redirect("/panel")
        u=Usuario(username=request.form['username'].strip(), password=generate_password_hash(request.form['password']), rol=request.form['rol'], nombre=request.form['nombre'].strip()[:100], precio_agua=max(0,float(request.form.get('precio_agua',22))))
...
@login_required
@rol_required(['sistemas']) # FIX: solo sistemas puede borrar
def borrar(id):
    try:
...
def reset(id):
    try:
        ...
        if u and u.username!='sistemas':
            u.password=generate_password_hash('123456'); db.session.commit()
    except: db.session.rollback()
    return redirect("/panel")
...
def up_loc():
    try:
        ...
        if not (-90<=lat<=90 and -180<=lng<=180): return jsonify(ok=False)
...
# --- HTML igual pero con FIX MAPA ---
LOGIN_HTML = """<body style="margin:0;font-family:system-ui;background:#e0f7fa;display:flex;justify-content:center;align-items:center;height:100vh"><div style="background:white;padding:35px;border-radius:20px;width:360px;text-align:center;box-shadow:0 8px 24px rgba(0,0,0,.1)"><img src="/logo.png" style="width:180px" onerror="this.outerHTML='<div style=\\'background:#0891b2;color:white;padding:12px;border-radius:12px;font-weight:bold\\'>💧 GOTA DE MANANTIAL<br><small>Mexicali</small></div>'"><h2>GOTA DE MANANTIAL v11</h2>{% if error %}<div style="background:#fee2e2;color:#991b1b;padding:8px;border-radius:8px;margin-bottom:8px">{{error}}</div>{% endif %}<form method="post"><input name="username" placeholder="Usuario" required style="width:100%;padding:12px;margin:6px 0;border-radius:8px;border:1px solid #ccc"><input name="password" type="password" placeholder="Contrasena" required style="width:100%;padding:12px;margin:6px 0;border-radius:8px;border:1px solid #ccc"><button style="width:100%;padding:12px;background:#0891b2;color:white;border:none;border-radius:8px;font-weight:bold">Entrar</button></form></div></body>"""

ADMIN_HTML = """
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"/><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
<style>.pedido-row{border-bottom:1px solid #eee;padding:10px 8px;font-size:13px;display:flex;justify-content:space-between;align-items:center}.menu-btn{cursor:pointer;padding:4px 8px;border-radius:6px;font-weight:bold;font-size:18px}.menu-btn:hover{background:#f1f5f9}.dropdown{position:absolute;right:0;top:28px;background:white;border:1px solid #ddd;border-radius:8px;box-shadow:0 4px 12px rgba(0,0,0,.15);min-width:160px;display:none;z-index:99}.dropdown a{display:block;padding:8px 12px;text-decoration:none;color:#333;font-size:13px}.dropdown a:hover{background:#f1f5f9}</style>
<body style="margin:0;font-family:system-ui;background:#f1f5f9"><div style="background:white;padding:10px 14px;display:flex;justify-content:space-between;align-items:center"><div style="display:flex;align-items:center;gap:8px"><img src="/logo.png" style="height:38px" onerror="this.outerHTML='<b>💧 GOTA MXLI v11</b>'"><b>{{nombre}} ({{rol}})</b></div><a href="/logout">Salir</a></div>
<div style="padding:12px;max-width:1200px;margin:auto;display:grid;grid-template-columns:1fr 360px;gap:12px"><div>
...
<div style="background:#fffbeb;border:2px solid #f59e0b;border-radius:12px;padding:12px;margin-bottom:12px"><h3>Solicitudes de precio</h3>{% for s in solicitudes %}<div style="background:white;border:1px solid #fbbf24;padding:8px;border-radius:8px;margin-bottom:6px;font-size:13px"><b>{{s.cantidad}}x {{s.tamano|e}} ${{s.precio_estaba}}->${{s.precio_quedo}} = ${{s.diferencia}}</b><br>{{dict.get(s.chofer_id,'-')|e}} - {{s.direccion|e}} - {{s.motivo|e}}<br><a href="/autorizar/{{s.id}}" style="background:#16a34a;color:white;padding:5px 10px;border-radius:5px;text-decoration:none">Aceptar</a> <a href="/rechazar/{{s.id}}" style="background:#dc2626;color:white;padding:5px 10px;border-radius:5px;text-decoration:none">Rechazar</a></div>{% else %}<p style="font-size:13px;color:#666">Sin solicitudes</p>{% endfor %}</div>
<div style="background:white;border-radius:12px;padding:12px"><h3>Pedidos</h3>{% for p in pedidos %}<div class="pedido-row"><span><b>{{p.cliente|e}}</b> - {{p.direccion|e}} - {{p.cantidad}}x ${{p.precio}} - <b>{{p.estado|e}}</b> - <span style="color:#0891b2">{{dict.get(p.chofer_id,'Sin asignar')|e}}</span></span><div style="position:relative"><div class="menu-btn" onclick="toggleMenu({{p.id}})">⋮</div><div id="menu-{{p.id}}" class="dropdown"><a href="/pedido/entregar/{{p.id}}">✅ Entregado</a><div style="border-top:1px solid #eee;padding:6px 12px;font-size:11px;color:#888">Mandar a:</div>{% for ch in choferes %}<a href="/pedido/mover/{{p.id}}?chofer_id={{ch.id}}">👤 {{ch.nombre|e}}</a>{% endfor %}<a href="/pedido/mover/{{p.id}}?chofer_id=0" style="color:#888">📦 Sin asignar</a></div></div></div>{% endfor %}</div></div>
<div><div style="background:white;border-radius:12px;padding:12px;margin-bottom:12px"><h3>Mapa Mexicali</h3><div id="map" style="height:320px;border-radius:8px"></div></div>
<div style="background:white;border-radius:12px;padding:12px"><h3>Empleados</h3><form action="/usuario/nuevo" method="post" style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:8px"><input name="nombre" placeholder="Nombre" required><input name="username" placeholder="Usuario" required><input name="password" placeholder="Pass" required style="width:60px"><select name="rol"><option value="chofer">Chofer</option><option value="admin">Admin</option></select><input name="precio_agua" type="number" value="22" style="width:45px"><button>Crear</button></form>{% for u in usuarios %}<div style="border-bottom:1px solid #eee;padding:5px;font-size:12px"><b>{{u.nombre|e}} ({{u.rol|e}})</b> {% if u.rol=='chofer' %} ${{u.precio_agua}} <form action="/usuario/precio/{{u.id}}" method="post" style="display:inline"><input name="precio_agua" value="{{u.precio_agua}}" style="width:35px"><button>OK</button></form>{% endif %} <a href="/usuario/reset/{{u.id}}" onclick="return confirm('Reset a 123456?')">Reset</a> {% if rol=='sistemas' %}<a href="/usuario/borrar/{{u.id}}" style="color:red" onclick="return confirm('Seguro borrar?')">Baja</a>{% endif %}</div>{% endfor %}</div></div></div>
<script>
function toggleMenu(id){document.querySelectorAll('.dropdown').forEach(d=>{if(d.id!=='menu-'+id) d.style.display='none'}); var m=document.getElementById('menu-'+id); m.style.display = m.style.display==='block' ? 'none' : 'block';}
...
CHOFER_HTML = """
<body style="margin:0;font-family:system-ui;background:#f0fdf4"><div style="background:white;padding:10px 14px;display:flex;justify-content:space-between"><div style="display:flex;align-items:center;gap:8px"><img src="/logo.png" style="height:35px" onerror="this.outerHTML='<b>💧 MXLI v11</b>'"><b>{{nombre|e}} - ${{precio_usar}} | Corte ${{total_venta}}</b></div><a href="/logout">Salir</a></div>
<div style="padding:12px;max-width:700px;margin:auto"><div style="background:white;padding:12px;border-radius:12px;margin-bottom:12px"><h3>Mi Corte Hoy - Vendidos {{vendidos}}</h3><form action="/corte/update" method="post" style="display:flex;gap:6px">Salio <input name="salio" value="{{corte.salio}}" type="number" style="width:60px" min="0"> Regreso <input name="regreso" value="{{corte.regreso}}" type="number" style="width:60px" min="0"><button>Guardar</button></form><p>Vendidos {{vendidos}} x ${{precio_usar}} - Desc ${{total_desc}} = <b>${{total_venta}}</b></p><form action="/descuento/add" method="post" style="display:flex;gap:4px;flex-wrap:wrap;margin-top:8px"><select name="tipo"><option value="tambo">Tambo</option><option value="botella">Botella</option></select><input name="cantidad" type="number" value="1" style="width:40px" min="1"><input name="tamano" placeholder="Motivo"><input name="total" type="number" placeholder="$" style="width:50px" min="0" max="500"><button>Descuento</button></form>{% for d in descuentos %}<div style="font-size:12px;border-bottom:1px solid #eee">{{d.cantidad}}x {{d.tamano|e}} - ${{d.total}} <a href="/descuento/borrar/{{d.id}}" style="color:red">x</a></div>{% endfor %}</div>
<div style="background:white;padding:12px;border-radius:12px;margin-bottom:12px"><h3>Solicitar precio especial</h3><form action="/solicitar_precio" method="post" style="display:flex;gap:5px;flex-wrap:wrap"><input name="tamano" placeholder="20L" required style="width:60px"><input name="cantidad" type="number" value="1" style="width:40px" min="1"><input name="estaba" type="number" placeholder="Estaba" required style="width:60px" min="1"><input name="quedo" type="number" placeholder="Quedo" required style="width:60px" min="1"><input name="direccion" placeholder="Direccion" required><input name="motivo" placeholder="Motivo"><button style="background:#f59e0b;color:white;border:none;padding:6px 10px;border-radius:6px">Solicitar</button></form></div>
<div style="background:white;padding:12px;border-radius:12px"><h3>Mis pedidos</h3>{% for p in pedidos %}<div style="border-bottom:1px solid #eee;padding:8px;font-size:13px;display:flex;justify-content:space-between"><span>{{p.cliente|e}} - {{p.direccion|e}} - {{p.cantidad}}x ${{p.precio}} - {{p.estado|e}}</span><a href="/pedido/entregar/{{p.id}}" style="background:#16a34a;color:white;padding:4px 8px;border-radius:4px;text-decoration:none">Entregar</a></div>{% endfor %}</div></div>
<script>if(navigator.geolocation){setInterval(()=>{navigator.geolocation.getCurrentPosition(pos=>{fetch('/chofer/update_location',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({lat:pos.coords.latitude,lng:pos.coords.longitude})})})},15000)}</script></body>"""
